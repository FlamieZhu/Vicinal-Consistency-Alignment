...
# License terms: LICENSES/LDAM_DRW.txt in the repository root.
'''
...
class ResNet_s(nn.Module):
    ...
    def __init__(self, block, num_blocks, num_classes=10, reduce_dimension=False, layer2_output_dim=None, layer3_output_dim=None, use_norm=False, s=30, no_fc=False):
        ...
        if no_fc:
            ...
            self.linear = NormedLinear(layer3_output_dim, num_classes, s=s)
        else:
            s = 1
...
def resnet32(num_classes=10, use_norm=False, no_fc=False):
    return ResNet_s(BasicBlock, [5, 5, 5], num_classes=num_classes, use_norm=use_norm, no_fc=no_fc)
...
class ResNetEMA(nn.Module):
    def __init__(self, base_model=resnet32, num_classes=100, use_norm=True, m=0.99):
        super(ResNetEMA, self).__init__()
        self.encoder_q = base_model(num_classes=num_classes, use_norm=use_norm, no_fc=True)
        self.encoder_k = base_model(num_classes=num_classes, use_norm=use_norm, no_fc=True)
        self.linear = NormedLinear(64, num_classes, s=30)
        self.linear_k = NormedLinear(64, num_classes, s=30)
        self.m = m
    ...
    def forward(self, x, training=True, teacher=False):

        if not training:
            lq = self.encoder_q(x)[0]
            return self.linear(lq)
        else:
            out_q, imd_out_q = self.encoder_q(x)
            lq = self.linear(out_q)
            
            with torch.no_grad():
                self._momentum_update_()
                out_k, imd_out_k = self.encoder_k(x)

                lk_all = self.linear_k(out_k)
