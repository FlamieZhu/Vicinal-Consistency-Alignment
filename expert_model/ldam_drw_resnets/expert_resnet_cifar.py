...
class ResNet_s(nn.Module):
    ...
    def __init__(self, block, num_blocks, num_experts, num_classes=10, reduce_dimension=False, layer2_output_dim=None, layer3_output_dim=None, use_norm=False, returns_feat=True, use_experts=None, s=30, hidden_dim=2048):
    ...
    def _separate_part(self, x, ind, separate=False):
        out = x
    ...
    def forward(self, x, separate=False):
        ...
        for ind in use_experts:
            outs.append(self._separate_part(out, ind, separate))
...
class ExpertRes32EMA(nn.Module):
    def __init__(self, num_classes=100, reduce_dimension=True, layer2_output_dim=None, layer3_output_dim=None, use_norm=False, num_experts=2, m=0.99, s=30):
        super(ExpertRes32EMA, self).__init__()
        self.m = m
        self.encoder_q = ResNet_s(BasicBlock, [5, 5, 5],num_classes=num_classes, reduce_dimension=reduce_dimension, use_norm=use_norm, num_experts=num_experts, s=s)
        self.encoder_k = ResNet_s(BasicBlock, [5, 5, 5], num_classes=num_classes, reduce_dimension=reduce_dimension, use_norm=use_norm, num_experts=num_experts, s=s)
